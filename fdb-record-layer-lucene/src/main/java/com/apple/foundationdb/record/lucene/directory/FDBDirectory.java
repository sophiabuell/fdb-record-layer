...
public class FDBDirectory extends Directory {
    ...
    private final FDBRecordContext fdbRecordContext;
    ...
    //TODO Doesn't the Nonnull check do its own verification?
    ...
    public synchronized long getIncrement() {
        return fdbRecordContext.ensureActive().get(sequenceSubspaceKey).thenApply(
            (value) -> {
                if (value == null) {
                    fdbRecordContext.ensureActive().set(sequenceSubspaceKey, Tuple.from(1L).pack());
                    return 1L;
                } else {
                    long sequence = Tuple.fromBytes(value).getLong(0) + 1;
                    fdbRecordContext.ensureActive().set(sequenceSubspaceKey, Tuple.from(sequence).pack());
                    return sequence;
                }
            }).join();
    }
    ...
    public CompletableFuture<FDBLuceneFileReference> getFDBLuceneFileReference(@Nonnull final String name) {
        ...
        if (fileReference == null) {
            return fdbRecordContext.ensureActive().get(metaSubspace.pack(name))
    ...
    public void writeFDBLuceneFileReference(@Nonnull final String name, @Nonnull final FDBLuceneFileReference reference) {
        ...
        fdbRecordContext.ensureActive().set(metaSubspace.pack(name), reference.getTuple().pack());
    ...
    public void writeData(long id, int block, @Nonnull byte[] value) {
        ...
        fdbRecordContext.ensureActive().set(dataSubspace.pack(Tuple.from(id, block)), value);
    }
    ...
    public CompletableFuture<byte[]> readBlock(@Nonnull String resourceDescription, @Nonnull CompletableFuture<FDBLuceneFileReference> referenceFuture, int block) throws IOException {
        try {
            ...
            return blockCache.get(Pair.of(id, block),
                    ...
                    return fdbRecordContext.ensureActive().get(dataSubspace.pack(Tuple.from(id, block)));
                }
            );
    ...
    public String[] listAll() {
        ...
        long totalSize = 0L;
        for (KeyValue kv : fdbRecordContext.ensureActive().getRange(metaSubspace.range())) {
    ...
    public void deleteFile(@Nonnull String name) throws IOException {
        ...
        boolean deleted = getFDBLuceneFileReference(name).thenApplyAsync(
                (value) -> {
                    ...
                    fdbRecordContext.ensureActive().clear(metaSubspace.pack(name));
                    fdbRecordContext.ensureActive().clear(dataSubspace.subspace(Tuple.from(value.getId())).range());
    ...
    public void rename(@Nonnull final String source, @Nonnull final String dest) {
        ...
        fdbRecordContext.ensureActive().get(key).thenAcceptAsync( (value) -> {
            this.fileReferenceCache.invalidate(source);
            fdbRecordContext.ensureActive().set(metaSubspace.pack(dest), value);
            fdbRecordContext.ensureActive().clear(key);
        }).join();
    }
